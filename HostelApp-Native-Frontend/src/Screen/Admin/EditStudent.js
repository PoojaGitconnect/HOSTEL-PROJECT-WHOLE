...
export default function EditStudent() {
  ...
  const { editStudent, isLoading, error, clearError, baseIp } = useContext(ApiContext);
  ...
  useEffect(() => {
    if (initialStudent) {
      setStudent({
        ...
        register_no: initialStudent.register_no?.toString() || '',
        room: initialStudent.room?.toString() || '',
        year: initialStudent.year?.toString() || '',
        student_class: initialStudent.class || '',
        gender: initialStudent.gender || 'Male',
        dob: initialStudent.dob ? new Date(initialStudent.dob) : new Date(),
        address: initialStudent.address || '',
        email: initialStudent.email || '',
        phone: initialStudent.phone || '',
        profile_picture: null,
        existing_picture: initialStudent.profile_picture || null,
      });
    }
  }, [initialStudent]);
    ...
    try {
      ...
      Object.keys(student).forEach(key => {
        if (key === 'profile_picture' && student[key]) {
          ...
          formData.append('class', student[key].toString());
        } else if (key !== 'profile_picture' && key !== 'existing_picture' && 
                   student[key] !== null && student[key] !== undefined && student[key] !== '') {
          formData.append(key, student[key].toString());
        }
      });

      console.log('Updating student data...');
      const response = await editStudent(initialStudent.id, formData);
  ...
  // Get current profile picture URI
  const getCurrentImageUri = () => {
    if (student.profile_picture?.uri) {
      return student.profile_picture.uri;
    }
    if (student.existing_picture) {
      return `${baseIp}/static/${student.existing_picture}`;
    }
    return 'https://via.placeholder.com/128/e5e7eb/6b7280?text=Photo';
  };
  ...
  return (
    ...
    >
      <AppHeader screenName="Edit Student" />
